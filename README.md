...
모든 일반 캐릭터 조합을 동일한 시드로 25회씩 대전시켜 기본 능력치 기준 승률을 출력합니다. 이 테스트는 스킬 훅·렌더링·사용자 입력을 제외한 **회귀 감지용 기준선**이므로, 결과만으로 실제 스킬 밸런스를 확정하지 않습니다. 자세한 해석 기준은 [밸런스 테스트 가이드](docs/BALANCE_TESTING.md)를 참고하세요.
