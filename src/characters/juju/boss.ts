...
const SKILL_CONSTANTS = {
  ...
  MAP_CUT_WARNING: 1.6,
  MAP_CUT_ACTIVE: 2.0,
  MAP_CUT_INTERVAL: 4.2,
  MAP_CUT_ENRAGE_INTERVAL: 3.0,
  MAP_CUT_DAMAGE: 9999,
...
// #endregion CONFIG
  ...
  onUpdate(char, dt, ctx) {
    ...
    if (state.bossPhase === 1) {
      ...
      const cutPoint = randomPoint(ctx);
      const sliceAngles = [0, Math.PI / 2, Math.PI / 4, -Math.PI / 4];
      const cutAngle = sliceAngles[Math.floor(Math.random() * sliceAngles.length)];
      ctx.spawnMapCut(char, { x: cutPoint.x - SKILL_CONSTANTS.MAP_CUT_WIDTH / 2, y: cutPoint.y - SKILL_CONSTANTS.MAP_CUT_HEIGHT / 2, width: SKILL_CONSTANTS.MAP_CUT_WIDTH, height: SKILL_CONSTANTS.MAP_CUT_HEIGHT, angle: cutAngle, warningDuration: SKILL_CONSTANTS.MAP_CUT_WARNING, activeDuration: SKILL_CONSTANTS.MAP_CUT_ACTIVE, damage: SKILL_CONSTANTS.MAP_CUT_DAMAGE });
      state.faults.push(spawnFault(ctx, cutAngle));
