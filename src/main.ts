...
function getSkillGrowthRoadmapMarkup(progress: PveProgress): string {
  const unlocked = new Set(getUnlockedSkillLevels(progress));
  return `<section class="skill-growth-roadmap"><div><span class="eyebrow">SKILL UNLOCK ROADMAP</span><strong>Lv.5 단위 신규 스킬</strong><p>구현된 스킬만 장착할 수 있으며, 나머지 해금 칸은 업데이트 예정으로 유지됩니다.</p></div><div class="skill-growth-slots">${SKILL_UNLOCK_LEVELS.map((unlockLevel) => unlocked.has(unlockLevel)
    ? `<div class="skill-growth-slot unlocked"><em>Lv.${unlockLevel} · 해금 완료</em><strong>업데이트 예정</strong><small>스킬 제작 후 장착할 수 있습니다.</small></div>`
